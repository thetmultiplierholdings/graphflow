---
snippet: Bash slop catalog grounding the Deslop review dimension—quoting, strict mode, ls parsing, subshell traps, echo portability, each with the clean fix and its ShellCheck code.
---

# Bash slop catalog

AI-generated shell anti-patterns and their fixes, for the scripts under `scripts/` and the repo's hooks. Grounds the Deslop dimension in [REVIEW.md](../../REVIEW.md)—cite the item when flagging one of these in review.
...
Quote every variable expansion: `"$var"`, `"${array[@]}"`, `"$(command)"`. The `--` stops option parsing (protects against filenames starting with `-`).

## 2. Missing or incomplete `set -euo pipefail`

AI scripts either omit strict mode entirely or use partial `set -e` without `-u` and `-o pipefail`. Both are dangerous.
...
All three flags together. `set -e` alone is a half-measure—especially dangerous in scripts that pipe through `jq`/`yq`/`grep` where a failure on the left side is silently swallowed.
...
`ls` output is not machine-readable. Filenames with spaces, newlines, or special characters break everything.
...
The highest-consequence tell: a failed `cd` (typo, permissions) lets every following command—including `rm -rf`—run in the wrong directory.
...
`echo`'s flag behavior differs between the bash builtin and `/bin/echo` and isn't POSIX-portable.
