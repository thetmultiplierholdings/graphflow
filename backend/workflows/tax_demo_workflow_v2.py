"""tax_demo_workflow_v2 — file copy of tax_demo_workflow with ONE
behavioural edit: the calculator applies the corrected FY26/27 rate of 24%
(was 25%). TAX_RATE is a value dependency declared in hash_with, so only
calculate_tax (changed rate) and build_report (edited report literal) get new
code hashes — every other node keeps its hash, and existing engagements'
answers (human reviews included) keep matching when a workspace is repointed
at this version.
...
@node(output_kind="tax_calc", display_name="Calculator (mock): sum * 24%",
      hash_with=[TAX_RATE])
...
def build_report(statements, slips, master, calc):
    ...
    lines.append(f"  {'TAX DUE (total * 24%)':<40} {c['tax_due']:>12}")
    pages.append("\n".join(lines))
...
@workflow_def(
    ...
    display_name="Tax demo workflow v2",
    kinds=[
