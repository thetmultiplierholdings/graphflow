...
const EXTRA: readonly (readonly [string, string])[] = [
  ['extra_ubs.txt', 'brokerage_statement'],
  ['extra_payslip_apr.txt', 'payment_slip'],
];

// Reads sample_docs cwd-relative; the CLI must run from backend_typescript/.
function supplyAndAttach(
  ...
  attach(conn, workflowRunId, ref.artifact_id, { source: 'user', addedBy: 'demo-user' });
...
function printReport(conn: Database.Database, storageRoot: string, workflowRunId: number, tag: string): void {
  const reports = workspaceArtifacts(conn, workflowRunId).filter((a) => a.kind === 'final_report');
...
export async function cmdDemo(env: Env): Promise<void> {
  ...
  try {
    const eng = createEngagement(conn, `acme-demo-${instance}`);
    out(`\n== SCENARIO 1: January from scratch (engagement ${eng}) ==`);
    const jan = createWorkspace(conn, eng, 'tax_demo_workflow', 'January estimate', { createdBy: 'demo-user' });
    for (const f of BROKERAGE) {
      supplyAndAttach(conn, env.storageRoot, eng, jan, f, 'brokerage_statement');
    }
    for (const f of SLIPS) {
      supplyAndAttach(conn, env.storageRoot, eng, jan, f, 'payment_slip');
    }
    const summary = await executeWorkspace(client, env.dbPath, jan);
    ...
    const summary2 = await executeWorkspace(client, env.dbPath, jan);
    ...
    const summary3 = await executeWorkspace(client, env.dbPath, feb);
...
export async function cmdSeed(env: Env, fresh: boolean): Promise<void> {
  ...
  try {
    ...
    try {
      ...
      const acme = createEngagement(conn, 'Acme Ltd — UK Tax FY 2025/26');
      out(`\n  [seed] engagement ${acme}: Acme Ltd — UK Tax FY 2025/26`);
      const jan = createWorkspace(conn, acme, 'tax_demo_workflow', 'January estimate', { createdBy: 'thet' });
      for (const f of BROKERAGE) {
        supplyAndAttach(conn, env.storageRoot, acme, jan, f, 'brokerage_statement');
      }
      for (const f of SLIPS) {
        supplyAndAttach(conn, env.storageRoot, acme, jan, f, 'payment_slip');
      }
      const summary = await executeWorkspace(client, env.dbPath, jan);
      printSummary('seed/January', summary);

      const reports = workspaceArtifacts(conn, jan).filter((a) => a.kind === 'final_report');
      const lastReport = reports.at(-1);
      if (lastReport !== undefined) {
        renameArtifact(conn, lastReport.artifact_id, 'January estimate — sent to client');
      ...
      const bh = createEngagement(conn, 'Blue Harbour LLP — UK Tax FY 2025/26');
      out(`\n  [seed] engagement ${bh}: Blue Harbour LLP — UK Tax FY 2025/26`);
      const q1 = createWorkspace(conn, bh, 'tax_demo_workflow_v2', 'Q1 estimate', { createdBy: 'thet' });
      supplyAndAttach(conn, env.storageRoot, bh, q1, 'bh_schwab.txt', 'brokerage_statement');
      supplyAndAttach(conn, env.storageRoot, bh, q1, 'bh_payslip_feb.txt', 'payment_slip');
      const handle = await startWorkspace(client, env.dbPath, q1, false);
